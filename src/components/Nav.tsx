...
export default function Nav() {
  ...
  const navLinks = [
    ...
    { label: 'Newsletters', href: '/editorials' },
  ]
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-[auto_1fr_auto] items-center gap-1 px-4 py-3 sm:px-6 sm:py-4 lg:px-8">
          ...
          <div className="flex items-center gap-1 justify-self-end">
            <div className="hidden items-center gap-1 sm:flex">
              <Link to="/sign-in" className="inline-flex min-h-[44px] items-center rounded-lg px-3 text-[0.9375rem] font-medium tracking-wide text-white/60 hover:text-white transition-colors">Sign in</Link>
              <Link to="/sign-up" className="ml-1 inline-flex h-9 items-center rounded-full bg-white px-4 text-[0.85rem] font-medium text-[#060a10] hover:bg-white/90 transition-colors">Sign up</Link>
            </div>
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-[#060a10]/80 backdrop-blur-sm border-b border-white/[0.08]">
      <div className="mx-auto grid max-w-7xl grid-cols-[auto_1fr_auto] items-center gap-1 px-4 py-3 sm:px-6 sm:py-4 lg:px-8">
        ...
        <div className="flex items-center gap-1 justify-self-end">
          <div className="hidden items-center gap-1 sm:flex">
            <Link to="/sign-in" className="inline-flex min-h-[44px] items-center rounded-lg px-3 text-[0.9375rem] font-medium tracking-wide text-white/50 hover:text-white transition-colors">Sign in</Link>
            <Link to="/sign-up" className="ml-1 inline-flex h-9 items-center rounded-full bg-white px-4 text-[0.85rem] font-medium text-[#060a10] hover:bg-white/90 transition-colors">Sign up</Link>
          </div>
          <MobileMenuBtn open={mobileOpen} onClick={() => setMobileOpen(!mobileOpen)} />
        </div>
    ...
    <div className="sm:hidden border-t border-white/[0.08] bg-[#060a10]/95 backdrop-blur-xl">
      <div className="px-4 py-5 flex flex-col gap-1">
        ...
        <div className="pt-3 mt-2 border-t border-white/[0.06] flex flex-col gap-2">
          <Link to="/sign-in" onClick={onClose} className="inline-flex h-11 items-center justify-center rounded-lg border border-white/15 text-sm font-medium text-white/70 hover:text-white transition-colors">Sign in</Link>
          <Link to="/sign-up" onClick={onClose} className="inline-flex h-11 items-center justify-center rounded-full bg-white text-sm font-medium text-[#060a10] hover:bg-white/90 transition-colors">Sign up</Link>
        </div>
      </div>
    </div>
  )
